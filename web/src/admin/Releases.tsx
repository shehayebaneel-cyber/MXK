...
const blank: Draft = { title: "", type: "SINGLE", featuredArtists: "", description: "", releaseDate: asDate(new Date().toISOString()), isFeatured: false, artwork: "" };
...
export function AdminReleases() {
  ...
  const load = () => api.get<Release[]>("/api/releases?all=1").then(setList).catch(() => {});
  useEffect(() => { load(); }, []);
  ...
  async function remove(r: Release) {
    ...
    await api.delete(`/api/releases/${r.id}`); await load();
  }

  return (
    <div>
      <PageTitle title="Music" action={<button onClick={() => setDraft({ ...blank })} className={btnPrimary}>+ New release</button>} />
          ...
          <div className="grid gap-4 sm:grid-cols-2">
            ...
            <Field label="Type">
              <select className={inputCls} value={draft.type} onChange={(e) => set("type", e.target.value)}>
                {TYPES.map((t) => <option key={t} value={t} className="bg-ink-3">{t.replace("_", " ")}</option>)}
              </select>
            </Field>
            <Field label="Featured artists"><input className={inputCls} placeholder="feat. …" value={draft.featuredArtists ?? ""} onChange={(e) => set("featuredArtists", e.target.value)} /></Field>
      ...
      <div className="space-y-2">
        {list.map((r) => (
          <div key={r.id} className="flex items-center gap-3 rounded-xl border border-line bg-ink-2 p-3">
            ...
            <div className="min-w-0 flex-1">
              <p className="truncate font-semibold text-chrome">{r.title} {r.isFeatured && <span className="ml-1 rounded bg-blue/20 px-1.5 py-0.5 text-[10px] font-bold text-blue">FEATURED</span>}</p>
              <p className="truncate text-xs text-fog">{r.type.replace("_", " ")} · {asDate(r.releaseDate)} {r.featuredArtists}</p>
            </div>
            <button onClick={() => setDraft({ ...r, releaseDate: asDate(r.releaseDate) })} className={btnGhost}>Edit</button>
            <button onClick={() => remove(r)} className="rounded-full px-3 py-2 text-sm text-red hover:bg-red/10">Delete</button>
          </div>
        ))}
